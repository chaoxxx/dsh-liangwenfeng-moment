...
- 当前是**波谷**（低谷计价时段）→ 模型按钮右侧显示**绿色向下 ▼**（梁文谷时刻）

跟随 DeepSeek API **峰谷计价**口径（默认北京时间）：

- 工作日低谷窗口 `00:30 – 08:30`（波谷），其余时间高峰（波峰）；
- 周末默认**全天低谷**（DeepSeek 周末统一按低谷价计费的规则）。
...
[ DeepSeek-V3.2 ⌄ ] ▼    ← 波谷（绿色向下箭头 = 梁文谷时刻，凌晨/周末）
```
...
- 下方附一行规则说明（时区 / 低谷窗口 / 周末全天低谷）；相位翻转时文案自动切换。

点开切换模型下拉后，DeepSeek 分组内的每个模型同样带时刻后缀。
...
例如想改成工作日 `12:00–14:00` 低谷，把 `LWFG_CONFIG.valleyStart/valleyEnd` 改为 `'12:00' / '14:00'`，
重新 `node scripts/build-client.mjs` 并重装/覆盖 `lib/client.js` 后重载即可。
...
- 不修改会话、模型或消息任何数据，纯展示；
- 判峰谷所用“波峰/波谷”与 DeepSeek 官方峰谷计价时段一致，具体窗口可自行按最新公告调整。
